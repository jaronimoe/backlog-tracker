...
- **Editing sessions is always local:** Steam's playtime lives on the appid link, not in sessions, so shrinking or deleting any session only changes your own logged time — every session uses the same plain Delete confirmation. A 0-minute marker whose note says "Last played on Steam" may reappear on the next sync if Steam still reports that day as last-played; it carries no minutes.
...
- Optional **Re-sync playtime** checkbox (Settings, beside the import button): when checked, already-linked games are refreshed instead of skipped — the queue shows how Steam's total moved (e.g. `+3h`) and `last_played_override` is bumped if Steam reports a more recent session. Manually logged sessions are untouched. Unchecked (default) keeps the skip-only behaviour above.
- **Per-game sync:** a Steam-linked game's detail screen shows a **"Sync playtime from Steam"** button (only when the game has a linked appid). It fetches the library, refreshes just that game (same logic as bulk re-sync) across every appid linked to it, and reports the change (`synced` the first time a total is recorded, `already up to date` otherwise). If Steam credentials aren't set it offers to open Settings.
- **Playtime is its own source (v9):** Steam's lifetime total per appid is stored on `game_external_ids.playtime_minutes` (NULL = never synced) and is never folded into `imported_minutes` and never written into sessions. A game's playtime is `max(sum of its Steam totals, imported_minutes + logged sessions)` — both sides measure the same play, so the larger number is the truer one, and a re-sync can never double-count however often it runs. The detail header spells the two sides out under the total. The pre-v9 per-game watermark (`games.steam_synced_minutes`, migrations v6/v7), the attribution of playtime deltas to the last-played day, and the "Steam dump cleanup" keep/discard dialogs are all retired: the column survives only because migrations are append-only, and nothing reads or writes it.
- **"Last played" marker sessions:** when Steam reports an `rtime_last_played`, a **0-minute marker session** is stamped on that date (`INSERT OR IGNORE` on `UNIQUE(game_id, date)`) so the game surfaces on its last-played day without adding playtime or clobbering a real logged session. Markers are not sessions: they count towards play days and streaks, never towards session counts. Never-launched games (`rtime_last_played = 0`) get no marker.
